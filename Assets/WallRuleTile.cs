...
[CreateAssetMenu]
public class WallRuleTile : RuleTile<NewCustomRuleTile.Neighbor>
{
    ...
    {
        int newAngle = (int)m_DefaultGameObject.transform.rotation.eulerAngles.y;
            if (RuleMatches(rule, position, tilemap,newAngle))
            {
                transform = Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(0f, newAngle, 0f), Vector3.one);
            return true;
            }
